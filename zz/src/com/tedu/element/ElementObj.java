...
//	注明:当重新定义一个用于判定状态的变量,需要思考:1.初始化 2.值的改变 3.值的判定
	public ElementObj() { //这个构造其实没有作用,只是为了继承的时候不报错而写
		
	}
	/**
	 * @说明 带参数的构造方法,可以由子类传输到父类
	 * @param x 	左上角x坐标
	 * @param y 	左上角y坐标
	 * @param w 	w宽度
	 * @param h 	h高度
	 ...
	 * @说明 方式2 使用接口的方式；使用接口方式需要在监听类进行类型的转换
	 * @tips 约定 配置 现在大部分的java框架都是需要进行配置的
	 * 		 约定优于配置
	 * @param bl 点击的类型 true代表按下，false代表松开
	 * @param key 代表触发的键盘的code值
	 * @扩展 本方法是否可以分为两个方法？1个接收按下，1个接收松开（同学扩展使用）
	 */
	...
	/**
	 * @设计模式 模板模式：在模板模式钟定义对象执行方法的先后顺序，由子类选择性重写方法
	 * 		1.移动 2.换装 3.子弹发射
	 */
...
//		可以将这个数据进行处理
	...
	/**
	 * 只要是 VO类 就要为属性生成get和set方法 --POJO定义规范
	 */
